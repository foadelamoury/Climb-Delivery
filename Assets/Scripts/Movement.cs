...
public class Movement : MonoBehaviour
{
    ...
    public float wallJumpStaminaCost = 5;
    ...
    void Update()
    {
        ...
        if (coll.onWall && Input.GetButton("Fire3") && canMove && currentStamina > 0)
        {
        ...
        if (wallGrab && !isDashing)
        {
            ...
            rb.linearVelocity = new Vector2(rb.linearVelocity.x, y * (climbSpeed * speedModifier));

            // Stamina consumption
            float consumptionRate = y > 0 ? 5 : 2;
            ...
            {
                anim.sr.color = Color.Lerp(Color.white, Color.red, Mathf.PingPong(Time.time * 10, 1));
            }
            else
            {
        ...
        if (Input.GetButtonDown("Jump"))
        {
            ...
            if (coll.onWall && !coll.onGround)
            {
                if (currentStamina >= wallJumpStaminaCost)
                {
                ...
                {
                    // Optional: Weak wall jump when out of stamina
                    WallJump();
                }
            }
        }
